...
pub fn execute(path: impl AsRef<Path>) -> Result<String, WizError> {
    ...
    let lua = Lua::new();
    let mut result: Result<String, WizError> = Ok(String::default());
    lua.context(|ctx| {
...
fn to_lua<T>(result: Result<T, WizError>) -> Result<T, rlua::Error> {
    match result {
...
fn parent(path: impl AsRef<Path>) -> Result<PathBuf, WizError> {
    let path = path.as_ref();
